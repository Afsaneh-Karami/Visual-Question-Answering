...
class VQA_DataLoader():
    ...
    IMAGES_PATH = r"D:\semester B\advanced machine learning\Assessement\vaq2.0.dataset-cmp9137-item1\val2014-resised"
...
class VQA_Classifier(VQA_DataLoader,Patches):
    epochs = 40
    learning_rate = 7e-5
    ...
    model_resnet50=False
    model_inceptionv3=False
    model_nasnet=False
    transformer_image=True
    model_mobilenet_v2=False
    ...
    def create_vision_encoder(self, num_projection_layers, projection_dims, dropout_rate):
        ...
        if self.model_resnet50 is True:
            cnn_layer=tf.keras.applications.resnet50.ResNet50(include_top=False,weights='imagenet',input_shape=self.IMAGE_SHAPE)(img_input)
            cnn_layer = layers.Dropout(0.1)(cnn_layer)
        elif self.model_inceptionv3 is True:
            cnn_layer=tf.keras.applications.inception_v3.InceptionV3(include_top=False,weights='imagenet',input_shape=self.IMAGE_SHAPE)(img_input)
            cnn_layer = layers.Dropout(0.8)(cnn_layer)
        elif self.model_nasnet is True:
            cnn_layer = tf.keras.applications.nasnet.NASNetMobile(include_top=False,weights='imagenet',input_shape=self.IMAGE_SHAPE)(img_input)
            cnn_layer = tf.keras.layers.Conv2D(filters=64, kernel_size=(3, 3), activation='relu', padding='same', kernel_regularizer=tf.keras.regularizers.l2(0.1))(cnn_layer)
        elif self.model_mobilenet_v2 is True:
            cnn_layer=tf.keras.applications.mobilenet_v2.MobileNetV2(include_top=False,alpha=1.0,weights='imagenet',input_shape=self.IMAGE_SHAPE)(img_input)
            cnn_layer = tf.keras.layers.Conv2D(filters=64, kernel_size=(3, 3), activation='relu', padding='same', kernel_regularizer=tf.keras.regularizers.l2(0.1))(cnn_layer)
    ...
    def create_transformer_encoder(self):
        batch_size = 128
        patch_size = 32
        projection_dim = 286
        image_size = 224  
        ...
        img_input = layers.Input(shape=self.IMAGE_SHAPE, name="image_input")
        patches = Patches(patch_size)(img_input)#(augmented)
        # Encode patches.
        ...
        model = keras.Model(inputs=img_input, outputs=features)
        model.summary()
        return img_input, representation
    ...
    def build_classifier_model(self):
        ...
        if self.transformer_image is True:
            img_input, vision_net = self.create_transformer_encoder()
        else:
    ...
    def test_classifier_model(self):
        ...
        num_correct_predictions_p = 0
        num_correct_predictions_n = 0
        num_false_predictions_p = 0
        num_false_predictions_n = 0
        ...
        for features, groundtruth in self.test_ds:
            ...
            for batch_index in range(0, len(groundtruth)):
                ...
                if str(groundtruth[batch_index]) == predicted_class=="[1 0]": 
                    num_correct_predictions_p += 1
                if str(groundtruth[batch_index]) == predicted_class=="[0 1]": 
                    num_correct_predictions_n += 1
                if str(groundtruth[batch_index]) =="[0 1]" and predicted_class=="[1 0]":
                    num_false_predictions_p +=1
                if str(groundtruth[batch_index]) =="[1 0]" and predicted_class=="[0 1]":
                    num_false_predictions_n +=1
        ...
        accuracy = (num_correct_predictions_p+num_correct_predictions_n)/num_classifications
        precision=num_correct_predictions_p/(num_correct_predictions_p+num_false_predictions_p)
        recall=num_correct_predictions_p/(num_correct_predictions_p+num_false_predictions_n)
        F_score=2*precision*recall/(precision+recall)
        specificity=num_correct_predictions_n/(num_false_predictions_p+num_correct_predictions_n)
        balanced_accuracy=(recall+specificity)/2
        print("TEST accuracy=%4f" % (accuracy))
        print("TEST precision=%4f" % (precision))
        print("TEST recall=%4f" % (recall))
        print("TEST F_score=%4f" % ( F_score))
        print("TEST specificity=%4f" % ( specificity))
        print("TEST balanced_accuracy=%4f" % (balanced_accuracy))
